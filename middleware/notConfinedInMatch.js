/**
 * Middleware: notConfinedInMatch
 * Expects in req.body: { courtId/CourtId, date/Date, time/Time }
 * - If a Match exists for the same court and date within ±2 hours of the given time,
...
const { Match, Sequelize } = db;

async function notConfinedInMatch(req, res, next) {
  try {
    // Accept either lowercase or capitalized keys
    let courtId = req.body.courtId || req.body.CourtId;
    let date = req.body.date || req.body.Date;
    let time = req.body.time || req.body.Time;

    if (!courtId || !date || !time) {
      return res.status(400).json({ message: 'courtId/CourtId, date/Date and time/Time are required' });
    }

    // validate courtId
    const id = Number(courtId);
    if (!Number.isInteger(id) || id <= 0) {
      return res.status(400).json({ message: 'courtId must be a positive integer' });
    }
    ...
    const checkRange = async (dateStr, startS, endS) => {
      ...
      return Match.findOne({ where: { [Op.and]: conditions } });
    };

    // Range within same day
    if (startSec >= 0 && endSec < 86400) {
      if (await checkRange(date, startSec, endSec)) {
        return res.status(400).json({ message: 'this court in this time is confined' });
      }
    } else {
      // Spans previous day
      if (startSec < 0) {
    ...
    console.error('notConfinedInMatch middleware error:', err);
    return res.status(500).json({ message: 'Server error while checking match confinement' });
  }
}

module.exports = notConfinedInMatch;
